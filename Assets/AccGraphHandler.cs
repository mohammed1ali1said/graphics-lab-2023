...
public class AccGraphHandler : MonoBehaviour
{
    ...
    {
        CreatePoint(new Vector2(0, Target.acc_stats[0]));
        for (int i = 0; i < Target.respawnCounter; i++)
        {
            if (i == 0)
            {
                CreatePoint(new Vector2(i, Target.acc_stats[i]));
            }
            else
            {
                CreatePoint(new Vector2(i, (Target.acc_stats[i] - Target.acc_stats[i - 1])));
            }
        }
        UpdateGraph();
    }
    ...
    private void Start()
    {
        ...
        PrepareGraph();
        ExampleFunction();
        g1.transform.position +=new Vector3 (0,125,0);
        g2.transform.position += new Vector3(0, -350, 0);
