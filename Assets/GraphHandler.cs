...
public class GraphHandler : MonoBehaviour
{
    ...
    {
        CreatePoint(new Vector2(0, Target.time_stats[0] ));
        for (int i = 0; i < Target.respawnCounter; i++)
        {
            if (i == 0)
            {
                CreatePoint(new Vector2(i, Target.time_stats[i]));
            }
            else
            {
                CreatePoint(new Vector2(i, (Target.time_stats[i] - Target.time_stats[i - 1])));
            }
        }
        UpdateGraph();
    }
    ...
    private Vector2 contentScale = new Vector2(0,0);
    ...
    private Vector2 activePointValue = new Vector2(0,0);
    ...
    private Vector2 initialMousePos = new Vector2(0,0);
